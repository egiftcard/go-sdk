...
func TestRejectIncorrectDomain(t *testing.T) {
	...
	sdk.Auth.UpdatePrivateKey(secondaryPrivateKey)
	_, err := sdk.Auth.Verify("test.egiftcard.cc", payload, nil)

	assert.Equal(t, err.Error(), "Expected domain 'test.egiftcard.cc' does not match domain on payload 'egiftcard.cc'")
}
...
func TestRejectTokenIncorrectDoain(t *testing.T) {
	...
	sdk.Auth.UpdatePrivateKey(secondaryPrivateKey)
	token, _ := sdk.Auth.GenerateAuthToken(domain, payload, nil)
	_, err := sdk.Auth.Authenticate("test.egiftcard.cc", token)

	assert.Equal(t, err.Error(), "Expected token to be for the domain 'test.egiftcard.cc', but found token with domain 'egiftcard.cc'")
}
